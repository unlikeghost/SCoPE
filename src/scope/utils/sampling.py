...
class SampleGenerator:
    def __init__(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            missing_cols = set(
                feature_columns + ([label_column] if isinstance(label_column, str) else label_column)) - set(
                data.columns)
            if missing_cols:
        ...
        if len(self.labels.shape) > 1:
            # For multidimensional labels, convert each row to a tuple and count
    ...
    def generate(self, num_samples: int = 5) -> Generator[
        ...
        for index in indices_order:
            ...
            # Initialize a dictionary to hold samples known for each class
            ...
            for cls in sorted(self.unique_classes):
                ...
                mask = mask[mask != index]  # Exclude the current index (target sample)
                mask = np.sort(mask)

                sampled_indices = self.rng.choice(mask, size=num_samples, replace=replace)

                # Optionally: prevent selection of the target sample (uncomment for strict exclusion)
                # while True:
                #     sampled_indices = self.rng.choice(mask, size=num_samples, replace=replace)
...
if __name__ == '__main__':
    ...
    for test_x, test_y, kw_samples in gen.generate(num_samples=2):
        ...
        print(f'known samples: {kw_samples}')
        print('\n')
