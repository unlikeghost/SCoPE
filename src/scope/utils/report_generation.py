...
def make_report(y_true: Union[list, np.ndarray], y_pred: Union[list, np.ndarray], y_pred_proba: Union[list, np.ndarray],
                save_path: str = None) -> dict:
    """
    Generates a comprehensive performance report for results, including
    ROC-related metrics, F1 scores, log_loss, and confusion matrix evaluation.
    ...
    try:
        ...
        print(f"Warning: Error calculating ROC curve: {e}")
    ...
    try:
        ...
        print(f"Warning: Error calculating AUC-PR: {e}")
        auc_pr = 0.0

    try:
        f1 = f1_score(y_true, y_pred)
    except Exception as e:
        print(f"Warning: Error calculating F1 score: {e}")
        f1 = 0.0

    try:
        f2 = fbeta_score(y_true, y_pred, beta=2)
    except Exception as e:
        print(f"Warning: Error calculating F2 score: {e}")
        f2 = 0.0
    ...
    try:
        ...
        print(f"Warning: Error calculating log loss: {e}")
        logloss = 1.0
    ...
    try:
        ...
        print(f"Warning: Error calculating  Matthews correlation coefficient: {e}")
        mcc = None
